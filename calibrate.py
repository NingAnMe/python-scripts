    ...
    def calibrate(self, in_file, geo_file, ary_vis_coeff_file=None):
        ...
        for i in range(19):
            ...
            if channel_name not in self.Dn:
                ...
                self.Dn[channel_name] = np.concatenate(
                    (self.Dn[channel_name], dn))
                self.Ref[channel_name] = np.concatenate(
                    (self.Ref[channel_name], ref))
        ...
        for i in xrange(self.Band):
            ...
            if channel_name not in self.SV:
                ...
                self.SV[channel_name] = np.concatenate(
                    (self.SV[channel_name], sv))
                self.BB[channel_name] = np.concatenate(
                    (self.BB[channel_name], bb))
    ...
    def calibrate(self):
        ...
        # 201303060020 - 今
            ...
            for i in xrange(19):
                ...
                slope_old = (self.dsl ** 2) * k2_old + \
                    self.dsl * k1_old + k0_old
                dn_new = ev_ref_l1 / slope_old + sv_dn_l1
                slope_new = (self.dsl ** 2) * k2_new + \
                    self.dsl * k1_new + k0_new
...
def dataset_extract(dataset, probe_count, probe_id, slide_step=10):
    ...
    # 滑动计算 avg 和 std
    avg_std_list = pb_calculate.rolling_calculate_avg_std(
        dataset_ext, slide_step)
    # 过滤标准差范围内的有效值
    dataset_valid = pb_calculate.filter_valid_value(
        dataset_ext, avg_std_list, 2)
    # 计算均值
