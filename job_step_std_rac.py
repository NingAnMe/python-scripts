...
def run_GEO_LEO(matching, ymd):
    ...
    nc = GSICS_NC_RAC(ymd, matching, opath)
...
class GSICS_NC_RAC(object):
    def __init__(self, ymd, matching, output_root_dir):
        ...
        self.nc_name = 'W_CN-CMA-NSMC,SATCAL+%s+GEOLEOIR,%s+%s-%s+%s_C_BABJ_%s%s_demo_01.nc' % \
                        ('RAC', self.sat1, self.sen1, self.sat2, self.sen2, self.ymd, self.hms)
...
#                 matchedpoint_nc = urljoin(MATCH_DIR, self.matching, nc_ymd + '.NC')
                ...
                dset = ncFile.groups[chan].variables['SimGeoRad']
                if dset.ndim == 1:
                    leoRad = dset[:]
                else:
                    leoRad = dset[:, 0]

                dset = ncFile.groups[chan].variables['GeoTime']
                if dset.ndim == 1:
                    GeoTime = dset[:]
                else:
                    GeoTime = dset[:, 0]
...
racCfg = ConfigObj(os.path.join(MainPath, 'FY2X_RAC.attr'))
# 配置不存在预警
